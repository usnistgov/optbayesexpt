...
def trace_sort(settings, measurements):
    """Combine measurements at identical obex values

    For obe runs with a single, discrete setting variable, a typical measurement will generate
    several measurements at many of the settings.  This routine combines measurements with
    commmon setting values
    Args:
        settings: (ndarray) Setting values
        measurements: (ndarray) measurement values

    Returns: (xbar, ybar, ysig) tuple.
        xbar: (ndarray) setting values (sorted, none repeated)
        ybar: (ndarray) average measurement value at each setting
        ysig: (ndarray) standard deviation of measurement values at each setting
    """

    # Sort the arrays by the setting values
    sortindices = np.argsort(settings)
    xarr = settings[sortindices]
    yarr = measurements[sortindices]
    ...
    for x, y in zip(xarr, yarr):
        ...
        if x != oldx:
            ...
            # same setting value, so just accumulate y values for the same x value
            ylist.append(y)
    # process the last accumulated batch
    xbar.append(oldx)
    ybar.append(np.mean(np.array(ylist)))
    sbar.append(np.std(np.array(ylist))/np.sqrt(len(ylist)))
...
def plot_obe_example(host_axes, obe_filename):
    ...
    xbar, ybar, sbar = trace_sort(xtrace, ytrace)

    # histogram
    axR.hist(xtrace, bins=len(xbar), zorder=1, range=(1.45, 4.55), color='lightblue')
...
def plot_scan_example(host_axes, scan_filename):
    ...
    pts = 1 / np.array(ysig)
    host_axes.scatter(xvals, ymean, s=pts**2, c='red', label='average & fit')
...
def plot_scan_traces(host_axes):
    ...
    legend = plt.legend((line3, line4), ("rms fit error", "fit sigma"), title="Scan & fit method",
                        loc=1)
    return legend
...
def make_duo_plot():
    ...
    axL, axR = plot_obe_example(ax1, 'obe010.txt')
...
if __name__ == "__main__":
    ...
    # n_scan, x0data, sigdata = readscansigs()
    # plt.hist(np.array(x0data)[:,-1], bins=20)
    # plt.show()
