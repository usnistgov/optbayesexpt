...
class OptBayesExpt(ExptModel, ProbDistFunc):
    ...
    ExptModel provides methods for evaluating an experimental model
        ...
        * To add the model, make a child class of BayesOptExpt and redefine the model_function
        method.
    ProbDistFunc provides a probability distribution function and methods for parameter
        probabilities
    ...
    # new functionality
    # recieve a new measurement value and uncertainty (ymeas, sigma) make at a setting (
    # onesetting) and update the pdf to incorporate the new information.

    def pdf_update(self, onesetting, ymeas, sigma=1):
        """
        Input a new measurement value and uncertainty (ymeas, sigma) make at a setting (
        onesetting) and update the pdf to incorporate the new information.
    ...
    def opt_setting(self):
        ...
        So, we calculate model outputs for a bunch of possible model parameters and see where
        the output varies the most.
        ...
        # Entropy counts probabilities that are neither 0 or 1  Sum[p(x) ln(p(x)], but doesn't
        # capture how those probability values are arranged
    ...
    def good_setting(self, pickiness=1):
        """
        Calculate a setting with a good probability of refining the pdf, but not necessarily
          the best.
        In opt_setting method, the measurements often concentrate on a few settings,
        ...
        :param pickiness  - a setting selection tuning parameter.  Pickiness=0 produces random
        settingss.  Large pickiness values emphasize the optimum settings.
